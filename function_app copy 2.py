...
def agent_httptrigger(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Get an existing agent by name/id
        agent = project_client.agents.get(agent_name=agent_id)
        logging.info(f"Retrieved agent: {agent.name} (ID: {agent.id})")
        ...
        # Create response with the agent
        ...
        # Auto-approve MCP approval requests (like knowledge_base_retrieve)
        max_iterations = 10
        iteration = 0
            ...
            if hasattr(response, 'output') and response.output:
                for item in response.output:
                    ...
                    if item_type == "mcp_approval_request":
                        ...
                        logging.info(f"Auto-approving MCP tool: {approval_request_id} - {tool_name}")
                        break

            if approval_request_id:
                # Submit approval using previous_response_id pattern
                response = openai_client.responses.create(
                ...
                logging.info(f"Submitted MCP approval: {approval_request_id}")
            else:
                # No approvals needed, break the loop
                break
